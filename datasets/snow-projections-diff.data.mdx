...
name: 'Projections of Snow Water Equivalent losses'
description: "Percent-change to future snow water equivalent modeled using the Land Information System framework and CMIP6 projections"
media:
...
layers:
  - id: snow-projections-diff
    ...
    compare:
      ...
      layerId: snow-projections-diff
      mapLabel: |
        ::js ({ dateFns, datetime, compareDatetime }) => {
          return `${dateFns.format(datetime, 'DD LLL yyyy')} VS ${dateFns.format(compareDatetime, 'DD LLL' 2011)}`;        
        }
    legend:
      type: gradient
